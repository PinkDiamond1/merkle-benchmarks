...
fn insert_256(c: &mut Criterion) {
    c.bench_function("kelvin_hamt_insert_random_kvs 256", |b| {
        b.iter(|| insert_random_kvs())
    });
}

fn insert_persist_random_kvs(store: &Store<Blake2b>) {
    let mut two3 = HAMT::<[u8; 32], [u8; 32], Blake2b>::new();
...
fn insert_persist_256(c: &mut Criterion) {
    ...
    c.bench_function("kelvin_hamt_insert_persist_random_kvs 256", |b| {
